                ...
                <table style="background: #fff;width: 100%;border:1px solid #111;border-collapse: collapse;">
                    ...
                    <tr>
                        <td style="padding: 8px;">
							...
							<p style="font-size: 12px;margin: 5px 0;"> <b>Payment status: </b> <span> Prepaid </span></p>
			...
			<td style="padding: 0 30px 0;">
				<table style="text-align:left;background: #fff;width: 100%;border:1px solid #111;border-top:0;border-collapse: collapse;">
					...
					<tr class="table-heading">
						...
						<th style="border: 1px solid #111;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">Discount</p></th>
						<th style="border: 1px solid #111;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">GST</p></th>
						<th style="border: 1px solid #111;padding: 0 8px;text-align:right"><p style="font-size: 12px;margin: 5px 0;">Amount</p></th>
					</tr> 
					@foreach($result['getsalesChildDetail'] as $key=>$sale)
					<tr> 
						<td style="border: 1px solid #111;border-top:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ $key+1 }}.</p></td>
						<td style="border: 1px solid #111;border-top:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;"><strong>{{ ucfirst($sale['product_name']) }}</strong></p></td>
						<td style="border: 1px solid #111;border-top:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ \App\Helpers\commonHelper::getpriceIconByCountry($sale['price'],$result['currency_id'])   }}</p></td>
						<td style="border: 1px solid #111;border-top:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ $sale['quantity'] }}</p></td>
						<td style="border: 1px solid #111;border-top:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ \App\Helpers\commonHelper::getpriceIconByCountry($sale['discount_amount'],$result['currency_id'])   }}</p></td>
						<td style="border: 1px solid #111;border-top:0;border-bottom:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ \App\Helpers\commonHelper::getpriceIconByCountry($sale['gst_amount'],$result['currency_id'])   }}</p></td>
						<td style="border: 1px solid #111;border-top:0;border-bottom:0;padding: 0 8px;text-align:right"><p style="font-size: 12px;margin: 5px 0;">{{ \App\Helpers\commonHelper::getpriceIconByCountry($sale['subtotal'],$result['currency_id']) }}</p></td>	
					</tr>
					@endforeach 
					<tr> 
						...
						<td style="border: 1px solid #111;padding: 0 8px;text-align:right"><p style="font-size: 12px;margin: 5px 0;">{{ \App\Helpers\commonHelper::getpriceIconByCountry($result['nettotal'],$result['currency_id']) }}</p></td> 
					</tr> 
					<tr class="table-heading">
						<th colspan="5" style="padding: 15px 8px;font-size:15px">Amount in Words: {{ \App\Helpers\commonHelper::convert_number_to_words(number_format($result['nettotal'])) }} Only</th>
					</tr> 
				</table>
			</td>
		</tr>
